/**
 * Git-panel trust fence: loopback always passes; a live paired-device cookie is
 * an additional allow path when the remote-web-ui service is present. The fence
 * is the only thing standing between a LAN client and the git mutation routes,
 * so it is tested directly rather than only through a route.
 */
import type { IncomingMessage } from 'node:http'
import { describe, expect, it, vi } from 'vitest'
import { isGitPanelAllowed } from '../src/host/access.ts'
...
describe('isGitPanelAllowed', () => {
  it('allows loopback without a pairing service', () => {
    expect(isGitPanelAllowed({} as never, request())).toBe(true)
  })

  it('refuses a LAN client when no pairing service is present', () => {
    const ctx = { get: () => undefined }
    expect(isGitPanelAllowed(ctx as never, request({
  ...
  it('allows a LAN client with a live paired-device cookie', () => {
    ...
    expect(isGitPanelAllowed(ctx as never, req)).toBe(true)
